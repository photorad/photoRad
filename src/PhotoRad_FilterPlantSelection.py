"""Performing filtering of plants based on DLI levels.
    Inputs:
        ...
        _dliFilterResult: The dliFilterResult output from the PhotoRad
        AnalyzePlantSelection component.
    Output:
        allOptions: Generates an indexed list of all the combinations(without
        repetition) of the plants.
        selection: Provides the selection made for a specific grid/space.
        selectionGrid: Provides a list of indicies corresponding to the selection
        made. This can be used to visualize the results through a mesh.
"""
...
def selectPlants(dliFilterResult, dliData):
    ...
    combinations = list(itertools.chain(
        *[list(itertools.combinations(plantList, len(plantList) - idx)) for \
    ...
    selection = tuple(
        sorted([val.plantInstance.name for val in plantDLICombo if val.selection]))
    selectionIndex = combinations.index(selection)
    gridSize = dliData.dataSize[0]
    selectionGrid = [selectionIndex] * gridSize

    return {'allOptions': comboWithID, 'selection': selection,
